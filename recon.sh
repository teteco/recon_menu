...
run_bug_bounty(){
  ...
  run_with_progress "GAU (histórico)"       bash -lc "echo '$DOMAIN' | gau --subs --providers wayback,commoncrawl,otx > '$TMP/urls/gau.txt' 2>/dev/null || true" &
  pid3=$!
  run_with_progress "waybackurls (arquivo)" bash -lc "echo '$DOMAIN' | waybackurls > '$TMP/urls/wayback.txt' 2>/dev/null || true" &
  pid4=$!
  if [[ $LIVE -gt 0 ]]; then
