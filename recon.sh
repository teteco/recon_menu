...
index 0000000000000000000000000000000000000000..51aa22d5644d812c1facd2196ea84a329f619b1b
--- /dev/null
+++ b/pentest.sh
@@ -0,0 +1 @@
+recon.sh
\ No newline at end of file
diff --git a/recon.sh b/recon.sh
index 4e944c6c689de5ae54b9a7a519b21142b749e378..e755655a734904eb4fc7e6cfce6ffbc34dac02af 100644
--- a/recon.sh
+++ b/recon.sh
@@ -1,58 +1,116 @@
 #!/usr/bin/env bash
...
@@ -62,326 +120,404 @@ url = sys.argv[1]; newval = sys.argv[2]
...
+  local LIVE
+  LIVE=$(countf "$TMP/subdomains_live.txt")
+  local LIVE_PCT=0
...
+  local GAU WB KAT
+  GAU=$(countf "$TMP/urls/gau.txt")
+  WB=$(countf "$TMP/urls/wayback.txt")
+  KAT=$(countf "$TMP/urls/katana.txt")
+  local URLS
+  URLS=$(countf "$BASE/urls/urls_all.txt")
...
+  fi
+  local WP_VULN
+  WP_VULN=$(grep -ic "vulnerab" "$BASE/intel/wpscan.txt" 2>/dev/null || true)
